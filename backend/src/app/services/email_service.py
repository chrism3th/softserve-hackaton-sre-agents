...
def _render_done(identifier: str, title: str) -> tuple[str, str]:
    subject = f"Tu issue ha sido completada — {identifier}"
    html = f"""
<!DOCTYPE html>
<html lang="es">
<body style="font-family:sans-serif;color:#1a1a1a;max-width:560px;margin:auto;padding:24px">
  <h2 style="color:#16a34a">✅ Issue completada</h2>
  <p>La siguiente issue ha sido marcada como <strong>Done</strong>:</p>
  <blockquote style="border-left:4px solid #16a34a;padding:8px 16px;margin:16px 0;color:#374151">
    <strong>{identifier}</strong> — {title}
  </blockquote>
  <p>Gracias por reportarla. El equipo ha terminado de trabajar en ella.</p>
  <hr style="border:none;border-top:1px solid #e5e7eb;margin:24px 0">
  <p style="font-size:12px;color:#6b7280">Este mensaje fue enviado automáticamente por el sistema de SRE.</p>
</body>
</html>
""".strip()
    return subject, html
...
def _render_ready_to_start(identifier: str, title: str, state: str) -> tuple[str, str]:
    subject = f"Tu issue está lista para comenzar — {identifier}"
    html = f"""
<!DOCTYPE html>
<html lang="es">
<body style="font-family:sans-serif;color:#1a1a1a;max-width:560px;margin:auto;padding:24px">
  <h2 style="color:#2563eb">🚀 Issue lista para comenzar</h2>
  <p>La siguiente issue ha pasado al estado <strong>{state}</strong>:</p>
  <blockquote style="border-left:4px solid #2563eb;padding:8px 16px;margin:16px 0;color:#374151">
    <strong>{identifier}</strong> — {title}
  </blockquote>
  <p>Tu solicitud ya puede ser trabajada. El equipo comenzará a atenderla pronto.</p>
  <hr style="border:none;border-top:1px solid #e5e7eb;margin:24px 0">
  <p style="font-size:12px;color:#6b7280">Este mensaje fue enviado automáticamente por el sistema de SRE.</p>
</body>
</html>
