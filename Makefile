...
.PHONY: install
install: ## Install backend deps locally (outside docker) with uv
...
.PHONY: test
test: ## Run backend unit tests in container
...
test-integration: ## Run backend integration tests (needs DB + Redis)
